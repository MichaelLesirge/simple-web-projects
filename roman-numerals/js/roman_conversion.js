...
const vinculumNumbersMap = new Map(
	[...numbersMap.entries()].map(([key, value]) => [key * 1000, value.replace(/./g, "$&" + combiningOverline)])
);
const vinculumSecondNumbersMap = new Map(
	[...numbersMap.entries()].map(([key, value]) => [
		key * 1000 * 1000,
		value.replace(/./g, "$&" + combiningOverline + combiningOverline),
	])
);
...
const clockGlyphsMap = new Map(
	clockGlyphs.map((char, i) => [
		char,
		numToRoman(i + 1, { zero: false, combined: false, specialCharters: false, lowercase: false, mode: "standard" })[0],
	])
);
...
export function toRules(glyphs, rules) {
	glyphs = glyphs
		.split("")
		.map((glyph) => (rules.specialCharters ? specialGlyphsMap : standardGlyphsMap).get(glyph) ?? glyph)
		.join("");
	...
	if (rules.mode === "apostrophus") {
		glyphs = glyphs
			...
			.map(
				(glyph) =>
					(rules.combined ? new Map() : rules.specialCharters ? apostrophusSymbolToSpecial : apostrophusSymbolToNormal).get(
						glyph
					) ?? glyph
			)
			.join("");
...
export function toStandard(glyphs) {
	...
	glyphs = glyphs
		.split("")
		.map((glyph) => standardGlyphsMap.get(glyph) ?? glyph)
		.join("");
	return glyphs;
}
...
export function romanToNum(glyphs) {
	...
	let combiningOverlineMultiplier = 1;

	for (const char of Array.from(glyphs).reverse()) {
		let value = keys.get(char.toUpperCase());

		if (char === combiningOverline) {
			combiningOverlineMultiplier *= 1000;
			continue;
		}
		...
		value *= combiningOverlineMultiplier;
		combiningOverlineMultiplier = 1;
