...
function getElementScale(el) {
	return Math.min(el.offsetWidth, el.offsetHeight) / 800;
}
...
class Entity {
	...
	constructor(type, speedMultiplier = 1, x = undefined, y = undefined) {
		this.speedMultiplayer = speedMultiplier * getElementScale(arena);
		this.canMove = true;
		...
		this.updateSizes();
		this.setPos(
			x ?? randInt(0, arena.offsetWidth - this.el.offsetWidth),
			y ?? randInt(0, arena.offsetHeight - this.el.offsetHeight)
		);
	...
	move() {
		...
		const scale = sliderValues.scale * getElementScale(arena);

		if (minDistanceToEnemy < 30 * scale) {
			this.setType(enemy.type);
			return;
		}

		let xChange = (randInt(-CONFIG.randomExtraSpeed * 10, CONFIG.randomExtraSpeed * 10) / 10) * scale;
		let yChange = (randInt(-CONFIG.randomExtraSpeed * 10, CONFIG.randomExtraSpeed * 10) / 10) * scale;
...
(() => {
	...
	function setEntityCount(v) {
		while (entities.length < v) {
			const entity = new Entity(
				entity_types[entities.length % entity_types.length],
				1 + (Math.random() - 0.5) * CONFIG.speedDifAmount
			);
	...
	function setScale(v) {
		v = v && v / 100;
		root.style.setProperty("--icon-scale", v * getElementScale(arena));
	...
	const scalePos = document.querySelector("#scale-entities-pos");
	...
	let arenaSize = [arena.offsetWidth, arena.offsetHeight];

	const resize = () => {
		const newSize = [arena.offsetWidth, arena.offsetHeight];

		if (scalePos.checked) {
			entities.forEach((entity) => {
				entity.setPos(entity.x * (newSize[0] / arenaSize[0]), entity.y * (newSize[1] / arenaSize[1]));
			});
		}
