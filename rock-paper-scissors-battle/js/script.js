...
class Entity {
	...
	move() {
		...
		if (nearExtinct) {
			xChange = Math.max(Math.min(xChange * CONFIG.nearExtinctSpeedMultipler, CONFIG.speed * CONFIG.nearExtinctSpeedMultipler * 2), -CONFIG.speed * CONFIG.nearExtinctSpeedMultipler * 2);
			yChange = Math.max(Math.min(yChange * CONFIG.nearExtinctSpeedMultipler, CONFIG.speed * CONFIG.nearExtinctSpeedMultipler), -CONFIG.speed * CONFIG.nearExtinctSpeedMultipler);
		} else {
			xChange = Math.max(
