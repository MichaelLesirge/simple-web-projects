import { startLoop, updateCanvasSizes, setHashAutoFocus } from "./canvasUtil.js";

const canvas = document.getElementById("solar-canvas");
updateCanvasSizes(canvas);

const ctx = canvas.getContext('2d');

const EARTH_ORBIT_RADIUS = 105;
const EARTH_RADIUS = 12;
const MOON_ORBIT_RADIUS = 28.5;
const MOON_RAIDUS = 3.5;

// TODO use these constants to make animation (sort of) accurate  
const YEAR_TIME_SECONDS = 1;
const EARTH_ROTATIONS_IN_YEAR = 365.24;
const MOON_ORBITS_IN_YEAR = 365.24 / 27.322;

const sun = new Image();
sun.src = `data:image/png;base64,`;

const moon = new Image();
moon.src = `data:image/png;base64,`;

const earth = new Image();
earth.src = `data:image/png;base64,`; 

let startTime;

function init() {
    startTime = new Date().getTime();
}

function clear() {
    ctx.clearRect(0, 0, canvas.width, canvas.height); // clear canvas
}

function draw() {
    const minSize = Math.min(canvas.width, canvas.height)

    const scaler = minSize / 300;
    const earthOrbitRadius = EARTH_ORBIT_RADIUS * scaler;
    const earthRadius = EARTH_RADIUS * scaler;
    const moonOrbitRadius = MOON_ORBIT_RADIUS * scaler;
    const moonRadius = MOON_RAIDUS * scaler;

    ctx.globalCompositeOperation = 'destination-over';

    ctx.fillStyle = 'rgba(0, 0, 0, 0.4)';
    ctx.strokeStyle = 'rgba(0, 153, 255, 0.4)';
    ctx.save();
    ctx.translate(canvas.width / 2, canvas.height / 2);

    // Earth
    const time = (startTime - new Date().getTime());
    
    const angle = ((2 * Math.PI) / 365.25) * time;

    ctx.rotate(angle);
    ctx.translate(earthOrbitRadius, 0);

    ctx.fillRect(0, -earthRadius, earthRadius + moonOrbitRadius + moonRadius, 2 * earthRadius); // Shadow

    ctx.save();
    ctx.rotate((2 * Math.PI) * (time * 365.25))
    ctx.drawImage(earth, -earthRadius, -earthRadius, earthRadius * 2, earthRadius * 2);
    ctx.restore();

    // Moon
    ctx.save();
    ctx.rotate(10 * angle);
    ctx.translate(0, moonOrbitRadius);
    ctx.drawImage(moon, -moonRadius, -moonRadius, moonRadius * 2, moonRadius * 2);
    ctx.restore();

    ctx.restore();

    ctx.beginPath();
    ctx.arc(canvas.width / 2, canvas.height / 2, earthOrbitRadius, 0, Math.PI * 2, false); // Earth orbit
    ctx.stroke();

    ctx.drawImage(sun, (canvas.width - minSize) / 2, (canvas.height - minSize) / 2, minSize, minSize)  // Sun
}

export default function solar() {
    setHashAutoFocus(canvas);
    startLoop(canvas, init, clear, draw, { resetOnClick: true, resetOnResize: false })
}
